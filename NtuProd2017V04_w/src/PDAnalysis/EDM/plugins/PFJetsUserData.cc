...
class PFJetUserData : public NtuEDConsumer<edm::EDProducer> {
...
private:
  ...
  //tokens
  NtuEDToken< reco::VertexCollection > gt_recVtxs;
  NtuEDToken< PFJetCollectionAB      > gt_jetColl;
  NtuEDToken< ValueMap<float>        > gt_qglMap;
...
PFJetUserData::PFJetUserData(const edm::ParameterSet &pSet){
  ...
  consume< reco::VertexCollection >( gt_recVtxs.token, edm::InputTag( "offlinePrimaryVertices" ) );
  consume< PFJetCollectionAB      >( gt_jetColl.token, jetLabel_ );
  consume< ValueMap<float>        >( gt_qglMap.token , qgMap_ );
...
//    iEvt.getByLabel(qgMap_,qglMap);
  ...
  std::vector< pat::Jet >* outputPFJets = new std::vector< pat::Jet >(*jetColl);
//  std::auto_ptr< std::vector< pat::Jet > > outputPFJets(new std::vector< pat::Jet >(*jetColl));
  ...
  iEvt.put( EDMTypeWriter< std::vector< pat::Jet > >::typePtr( outputPFJets ) );
